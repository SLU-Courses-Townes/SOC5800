...
dataAugmented %>%
  ...
  mutate(educationHighest = max(as.numeric(educationLevels),na.rm = FALSE)) %>%
  ...
  mutate(countServicesAfter = nchar(programsUsed)) %>%
  
  # Determine number of social services used during incarceration
  mutate(countHelpDuring = nchar(helpDuring)) %>%
  
  # Determine number of social services used after release
  mutate(countHelpAfter = nchar(helpAfter)) -> dataAugmented
```
...
percentages(as.numeric(dataClean$gender))
...
percentages(as.numeric(dataClean$ethnicityRace))
...
percentages(as.numeric(dataClean$religiousAffiliation))
...
percentages(as.numeric(dataClean$incomeInitial))

print("incomeLastYr; ordinal data")
table(as.numeric(dataClean$incomeLastYr))
percentages(as.numeric(dataClean$incomeLastYr))

print("selfEmployment; 1 - 0%, 2 - 1-25%, 3 - 26-50%, 4 - 51-75%, 5 - 75-99%, 6 - 100%")
table(as.numeric(dataClean$selfEmployment))
percentages(as.numeric(dataClean$selfEmployment))

print("foodInsecurity; 1 - yes, 0 - no")
table(as.number(dataClean$foodInsecurity))
percentages(as.numeric(dataClean$foodInsecurity))
```
...
dataClean %>%
  dplyr::select(incomeLastYr, incomeChange, povertyRatio, managingFinancially, savings, programsUsedBinary,
                ...
                gender, ethnicityRace, typeHometown) -> correlationDataset 
...
regressionIncome <- lm(as.numeric(incomeLastYr) ~ as.numeric(programsUsedBinary) + as.numeric(ageNow) +
                         ...
                         aceScore + gritScore + gender + ethnicityRace + typeHometown, 
...
regressionFinStat <- lm(as.numeric(managingFinancially) ~ as.numeric(programsUsedBinary) + 
                          ...
                          mspssScore + issTotalScore + aceScore + gritScore + gender + ethnicityRace +
                          typeHometown,  
                        data = dataClean, na.action = na.omit)
