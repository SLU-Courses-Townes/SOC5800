...
dataAugmented$ageNow <- (as.numeric(dataAugmented$ageNow)+17)
dataAugmented$ageRelease <- (as.numeric(dataAugmented$ageRelease)+17)
...
# Comparison of gritScore for cases grouped by financialStatus
...
aovGroup <- aov(gritScore ~ as.numeric(financialStatus), data = groups)
summary(aovGroup)
```
...
dataClean %>%
  dplyr::select(povertyRatio, financialStatus, ageNow, ageRelease, mspssScore, aceScore, gritScore) -> correlationDataset 
