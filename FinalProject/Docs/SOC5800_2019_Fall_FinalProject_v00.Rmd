...
# Coding for povertyLevel (created variable): 1 - above, 2 - below
dataClean %>%
  mutate(povertyLevel = if (povertyRatio >1) {1} else {2})-> dataClean
...
dataClean %>%
  dplyr::select(incomeLastYr, povertyRatio, povertyLevel, financialStatus, savings, ageNow, ageRelease, mspssScore, aceScore, gritScore) -> correlationDataset 
...
dataClean %>%
  dplyr::select(incomeLastYr, povertyRatio, povertyLevel, financialStatus, savings, ageNow, ageRelease, mspssScore, aceScore, gritScore) %>%
  mutate(savingsBinary = 3-as.numeric(savings)) -> dataBinary
dataClean$savings
